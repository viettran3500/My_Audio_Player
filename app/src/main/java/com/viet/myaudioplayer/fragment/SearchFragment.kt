...
class SearchFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        listSongSearchAdapter = ListSongSearchAdapter(requireContext(), MainActivity.listMusicSearch, songViewModel)
        view.recyclerViewSearch.layoutManager = LinearLayoutManager(context, RecyclerView.VERTICAL, false)
    ...
    fun getInfo(view: View){
        for (i in 0 until listSearch.size){
            MainActivity.listMusicSearch.add(SongInfo(listSearch[i].id!!, listSearch[i].name!!, listSearch[i].artist!!, "https://photo-resize-zmp3.zadn.vn/${listSearch[i].thumb}", listSearch[i].duration!!.toInt(), null, null))

            ApiService.apiService.getInfoMusic(listSearch[i].id!!).enqueue(object : Callback<DataInfo>{
                ...
                override fun onResponse(call: Call<DataInfo>, response: Response<DataInfo>) {
                    ...
                    if(dataInfo!= null && dataInfo.err == 0 && dataInfo.msg == "Success"){
                        ...
                        MainActivity.listMusicSearch[i].source = "http://api.mp3.zing.vn/api/streaming/audio/${MainActivity.listMusicSearch[i].id}/128"
